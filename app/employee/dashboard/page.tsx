    ...
    <AppShell title={`Good Morning, ${session.name}`} subtitle={`Here's your performance overview for ${now.toLocaleString("default", { month: "long" })}.`} variant="employee">
      <div className="grid gap-5 md:grid-cols-3">
        <div className="stat-card">
          ...
          <div className="mt-3 text-3xl font-bold text-slate-900">{activeClients}</div>
          <div className="text-xs text-slate-400">+{newClients} this month</div>
        </div>

        <div className="stat-card">
          <div className="text-sm text-slate-500">Follow-ups Today</div>
          <div className="mt-3 text-3xl font-bold text-slate-900">{followupsTodayCount.toString().padStart(2, "0")}</div>
          <div className="text-xs text-slate-400">{followupsTodayCount} pending</div>
        </div>

        <div className="stat-card">
          <div className="text-sm text-slate-500">Monthly Target</div>
          <div className="mt-3 text-3xl font-bold text-slate-900">₹{targetAmount.toLocaleString()}</div>
          <div className="text-xs text-slate-400">Achieved: ₹{achieved.toLocaleString()} ({achievedPct}%)</div>
        </div>
      </div>

      <div className="mt-6 grid gap-5 xl:grid-cols-2">
        <div className="card p-5">
          <h3 className="text-lg font-bold text-slate-900">MONTHLY SALES TARGET</h3>
          <div className="mt-4 text-sm text-slate-600">₹{achieved.toLocaleString()} / ₹{targetAmount.toLocaleString()}</div>
        ...
        <div className="card p-5">
          ...
          <div className="mt-4 text-sm text-slate-700">Active: {activeClients} • New this month: {newClients} • Conversion: {conversionRate}%</div>
          <div className="mt-4 text-sm text-slate-600">Quick Actions:</div>
          <div className="mt-3 flex gap-2">
            <a href="/employee/clients" className="btn">View Clients</a>
            <a href="/employee/follow-ups/new" className="btn btn-primary">Add Follow-up</a>
            <a href="/employee/sales" className="btn">View Sales</a>
          </div>
        </div>
      </div>
    </AppShell>

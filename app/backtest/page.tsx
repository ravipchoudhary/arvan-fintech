...
export default async function BacktestPage() {
  ...
  const strategies = await prisma.strategy.findMany({
    where: session.role === "CLIENT" ? { clientId: session.id } : {},
    orderBy: { updatedAt: "desc" },
    take: 10,
  });

  return (
    <AppShell title="Backtest" subtitle="Strategy performance review" variant={session.role === "CLIENT" ? "client" : session.role === "EMPLOYEE" ? "employee" : "admin"}>
      ...
      <div className="mt-6 rounded-3xl border border-slate-200 bg-white p-5">
        <div className="mb-4 text-lg font-semibold text-slate-900">Recent strategy backtest signal set</div>
